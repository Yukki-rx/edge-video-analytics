...
"""对 usb_cam 图像进行缩放、限帧和 JPEG 压缩，以供边缘端推理。"""
...
class ImageCompressor(object):
    def __init__(self):
        ...
        rospy.loginfo("Image compressor input: %s", input_topic)
        rospy.loginfo("Image compressor output: %s", output_topic)
        rospy.loginfo(
    ...
    def image_callback(self, message):
        ...
        output.format = "jpeg"
        # tostring() 同时兼容 ROS Melodic/Noetic 系统中常见的
        # Python 2 和 Python 3 版本。
...
def main():
    ...
    ImageCompressor()
    rospy.loginfo("Image compressor started")
    rospy.spin()
