# Third-party notices

This repository is a multi-license collection. A license assigned to one package
does not override the license or copyright of another package.

The root MIT License applies to project-authored material unless a package or
file carries a more specific license. In particular,
`endpoint_device/src/robot_vision` is MIT-licensed, while
`edge_device/src/edge_yolo_ros` is AGPL-3.0-only. The third-party components and
resources listed below are not relicensed by the root license.

## usb_cam

- Path: endpoint_device/src/usb_cam
- Version: 0.3.6
- Upstream: https://github.com/bosch-ros-pkg/usb_cam
- License: BSD 3-Clause

The original LICENSE, AUTHORS.md, CHANGELOG.rst, src/LICENSE, and source-file
copyright notices are retained with the package.

## Ultralytics YOLO and model weights

- Integration path: edge_device/src/edge_yolo_ros
- Python dependency: ultralytics
- Upstream: https://github.com/ultralytics/ultralytics
- Official licensing information: https://www.ultralytics.com/license
- Upstream licensing: AGPL-3.0 or an applicable Ultralytics Enterprise License

edge_yolo_ros is licensed under AGPL-3.0-only. The included .pt files remain
subject to the terms attached by their respective authors and distributors.
Do not assume that the package license transfers ownership of a model or its
training data.

## OpenCV Haar cascade data

- Path: endpoint_device/src/robot_vision/data/haar_detectors

The XML files contain their original Intel/OpenCV copyright and redistribution
terms. Those embedded notices must be retained.
